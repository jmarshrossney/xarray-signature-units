...
`xarray-signature-units` enables run-time validation and conversion of `xarray.DataArray` units
...
`xarray-signature-units` helps by providing a transparent and low-effort policy for the
...
`xarray-signature-units` can be installed directly from GitHub using `pip`, or tools
...
=== "pip"
    ...
    ```sh
    pip install "xarray-signature-units[cf] @ git+https://github.com/jmarshrossney/xarray-signature-units.git"
    ```
...
`xarray-signature-units` is a deliberately thin layer over
...
[issue](https://github.com/jmarshrossney/xarray-signature-units/issues) or open a
[pull request](https://github.com/jmarshrossney/xarray-signature-units/pulls) to suggest a change
or feature.
